...
object FoldableInstances {
  ...
  implicit def listFoldable = new Foldable[List] {
    override def foldr[A, B](xs: List[A])(init: B)(fx: (A, B) => B): B = {
      xs match {
        case Nil => init
        case z::zs => fx(z, foldr(zs)(init)(fx))
      }
    }
  }

  implicit def tuple2Foldable = new Foldable[({type E[X] = Tuple2[X, X]})#E] {
    override def foldr[A, B](xs: (A, A))(init: B)(fx: (A, B) => B): B = listFoldable.foldr(List(xs._1, xs._2))(init)(fx)
  }

  implicit def tuple3Foldable = new Foldable[({type E[X] = (X, X, X)})#E] {
    override def foldr[A, B](xs: (A, A, A))(init: B)(fx: (A, B) => B): B = listFoldable.foldr(List(xs._1, xs._2, xs._3))(init)(fx)
  }
